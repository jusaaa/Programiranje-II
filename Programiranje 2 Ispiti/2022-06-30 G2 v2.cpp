...
const char* IspisKarakteristika[]={ "NARUDZBA","KVALITET","PAKOVANJE","ISPORUKA" };
    ...
    void AddOcjenuKarakteristike(Karakteristike karakteristika, int ocjena) {
        for (size_t i = 0; i < _ocjeneKarakteristika->getTrenutno(); i++)
        {
            if (_ocjeneKarakteristika->getElement1(i) == karakteristika)
                throw exception("Data karakteristika je vec ocijenjena!");
        }
...
public:
 ...
 //*/
    void SetZadovoljstvoKupca(ZadovoljstvoKupca zadovoljstvoKupca) {
        ...
        int brojac = 0;
        for (size_t i = 0; i <zadovoljstvoKupca.GetOcjeneKarakteristika()->getTrenutno(); i++)
        {
            if (zadovoljstvoKupca.GetOcjeneKarakteristika()->getElement2(i) < 5)
                brojac++;
        }
        if (brojac >= 2) {
            thread thr(&Kupovina::PosaljiMail, this, zadovoljstvoKupca);
            thr.join();
        }
    }
    void PosaljiMail(ZadovoljstvoKupca zadovoljstvoKupca) {
        cuvar.lock();
        cout << "To: " << endl;
        for (size_t i = 0; i < _kupaci.size(); i++)
        {
        ...
        cout << "Subject: Informacija" << endl
            << "Postovani," << endl
            << "Zaprimili smo Vase ocjene, a njihova prosjecna vrijednost je " <<zadovoljstvoKupca.ProsjecnaOcjena() << endl
            << "Zao nam je zbog toga, te ce Vas u najkracem periodu kontaktirati nasa Sluzba za odnose sa kupcima." << endl
            << "Nadamo se da cete i dalje poslovati sa nama" << endl
            << "Puno pozdrava" << endl;
        cuvar.unlock();
    }
...
void main() {
   ...
   // */
    ...
    if (!ValidirajJedinstveniKod("BH2356{ih}"))
        cout << "Jedinstveni kod NIJE validan" << endl<<endl;

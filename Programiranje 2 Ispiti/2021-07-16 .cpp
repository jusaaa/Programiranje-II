...
public:
	...
	void AddElement(T1 el1, T2 el2, int lokacija) {
		if (*_trenutno == max)
			throw exception("Dosegli ste max vrijednost elemenata!");
...
//  evidentirane (bez obzira sto je stariji od 3 dana)
   //*/
	bool AddOcjena(int ocjena, Datum datum) {
		...
		{
			int razlika = datum.SumaDana() - _ocjene.getElement2(i)->SumaDana();
			if (razlika < 3)
				return false;
		}
	...
	float GetProsjecnaTehnika()const {
		if (_ocjene.getTrenutno() == 0)
			return 0;
		float temp = 0;
		for (size_t i = 0; i < _ocjene.getTrenutno(); i++)
		{
			temp += _ocjene.getElement1(i);
		}
		return temp / _ocjene.getTrenutno();
	}
	...
	friend ostream& operator<< (ostream& COUT, const Tehnika& obj) {
		...
		{
			COUT << "Ocjena: " << obj._ocjene.getElement1(i) << " dana " << *obj._ocjene.getElement2(i) << endl;
		}
		COUT << "Prosjek: " << obj.GetProsjecnaTehnika() << endl;
		return COUT;
	}
};
class Polaganje {
...
bool ValidirajLozinku(string lozinka) {
	return regex_match(lozinka, regex(".{7,}")) &&
...
public:
	Korisnik(const char* imePrezime, string emailAdresa, string lozinka)
	{
		...
		if (!ValidirajLozinku(lozinka))
			_lozinka = NIJE_VALIDNA;//inicijalizirati na nacin zahtijevan u zadatku
		else
			_lozinka = lozinka;
	}
...
//  tehnike za ZUTI pojas i njihov prosjek je veci od 3.5)
   ...
   //*/
	bool AddTehniku(Pojas pojas, Tehnika tehnika) {
		if (!ProvjeriNiziPojas(pojas, tehnika))
			return false;
	...
	bool ProvjeriNiziPojas(Pojas pojas,Tehnika tehnika) {
		if (pojas <= ZUTI)
			return true;
		for (size_t i = 0; i < _polozeniPojasevi.size(); i++)
		{
			if (pojas - 1 == _polozeniPojasevi[i]->GetPojas() && _polozeniPojasevi[i]->GetTehnike().size() >= 3) {
				for (size_t j = 0; j < _polozeniPojasevi[i]->GetTehnike().size(); j++)
				{
					if (_polozeniPojasevi[i]->GetTehnike()[j]->GetProsjecnaTehnika() < 3.5)
						return false;
				}
				return true;
			}

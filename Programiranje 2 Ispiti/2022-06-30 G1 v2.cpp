...
const char* IspisKriterija[] = { "CISTOCA","USLUGA","LOAKCIJA","UDOBNOST" };
...
public:
    ...
    bool ImaLiDuplikata(T1 el1, T2 el2) {
    ...
    void AddElement(T1 el1, T2 el2) {
        if (!_omoguciDupliranje && ImaLiDuplikata(el1, el2))
    ...
    Kolekcija<T1, T2>InsertAt(int lokacija, T1 el1, T2 el2){
        T1* temp1 = _elementi1;
        T2* temp2 = _elementi2;
...
public:
   ...
   //    Puno pozdrava
   //*/
    void SetKomentar(Komentar komenatarRezervacija) {
        _komentar = komenatarRezervacija;
        int brojac = 0;
        for (size_t i = 0; i < komenatarRezervacija.GetOcjeneKriterija()->getTrenutno(); i++)
        {
            if (komenatarRezervacija.GetOcjeneKriterija()->getElement2(i) < 5)
                brojac++;
        }
        if (brojac >= 2) {
            thread thr1(&Rezervacija::PosaljiMejl, this, komenatarRezervacija);
            thr1.join();
        }
    }
    void PosaljiMejl(Komentar komentar) {
        cuvar.lock();
        cout << "To:" << endl;
        ...
        cout << "Subject: Informacija" << endl
            << "Postovani," << endl
            << "Zaprimili smo Vase ocjene, a njihova prosjecna vrijednost je "<<komentar.ProsjecnaOcjena() << endl
            << "Zao nam je zbog toga, te ce Vas u najkracem periodu kontaktirati nasa Sluzba za odnose sa gostima." << endl
            << "Ugodan boravak Vam zelimo" << endl
            << "Puno pozdrava"<<endl<<endl; 
        cuvar.unlock();
    }
...
void main() {
   ...
   // */
    ...
    if (!ValidirajBrojPasosa("b1252 521"))
        cout << "Broj pasosa NIJE validan" << endl<<endl;

...
public:
	...
	Rjecnik<T1, T2>operator()(int OD, int DO) {
		Rjecnik<T1, T2>novi;
	...
	void DodajKomentarKarakteristike(Karakteristike karakteristika, const char* komentar) {
		for (size_t i = 0; i < _komentariKarakteristika.getTrenutno(); i++)
		{
			if (_komentariKarakteristika.getElement1(i) == karakteristika)
				throw exception("Data karakteristika je vec komentarisana!");
		}
...
// */
	void DodajKupovinu(float iznos, ZadovoljstvoKupca zadovoljstvoKupca) {
		...
		if (ostvareniBodovi > 5) {
			thread thr1(&Kupac::PosaljiMejl, this, ostvareniBodovi);
			thr1.join();
		}
	}
	void Info() {
	...
	void PosaljiMejl(int ostvareniBodovi) {
		cuvar.lock();
		cout << "Cekam 3 sekunde..." << endl;
		this_thread::sleep_for(chrono::seconds(3));

		cout << "To: " << GetEmail() << endl<<
		 "Subject: Ostvareni bodovi" << endl<<
		 "Postovani," << endl<<
		 " Prilikom posljednje kupovine ste ostvarili " << ostvareniBodovi << " bodova, tako da trenutno vas ukupan broj bodova iznosi "<<GetBodoviUkupno()<<"." <<endl<<
		 "Zahvaljujemo vam na kupovini." << endl<<
		 "Puno pozdrava" << crt;
		cuvar.unlock();
	}
	~Kupac()

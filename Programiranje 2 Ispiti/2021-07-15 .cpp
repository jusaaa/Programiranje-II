...
public:
    ...
    */
    bool AddOcjena(int ocjena, Datum datum) {
        ...
        {
            if (datum.SumaDana() - _ocjene->getElement2(i).SumaDana() < 3)
                return false;
        }
        _ocjene->AddElement(ocjena, datum);
        return true;
    }
    float GetProsjecnaTehnike()const {
    ...
    friend bool operator==(Tehnika& lijeva, Tehnika& desna) {
    ...
    friend ostream& operator<< (ostream& COUT, const Tehnika& obj) {
        ...
        COUT << "Prosjecna ocjena: " << obj.GetProsjecnaTehnike() << endl;
        return COUT;
    }
...
public:
    ...
    friend ostream& operator<< (ostream& COUT, const Polaganje& obj) {
        ...
        for (size_t i = 0; i < obj._polozeneTehnike.size(); i++)
            COUT << *obj._polozeneTehnike[i]<<endl;
        return COUT;
    }
};
///*
    ...
    bool ProvjeriNiziPojas(Pojas pojas, Tehnika tehnika) {
        ...
        for (size_t i = 0; i <_polozeniPojasevi.size(); i++)
        {
            if (pojas - 1 == _polozeniPojasevi[i].GetPojas() && _polozeniPojasevi[i].GetTehnike().size() >= 3) {
                for (size_t j= 0; j < _polozeniPojasevi[i].GetTehnike().size(); j++)
                {
                    if (_polozeniPojasevi[i].GetTehnike()[j]->GetProsjecnaTehnike() < 3.5)
                        return false;
                }
              return true;
            }
